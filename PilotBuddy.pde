...
float runwayLength;
Plane myPlane = new Plane(20, 20, 2.2, 1000, 400, 4000); //float pW, float pH, cross-sectional area (m^2), maximum velocity (m/s), mass (kg), thrust (N), drag coeff, lift-to-drag ratio (L/D). (Values of cessna 172) One pixel is 10m
void setup(){
  size(1500,600);
  createGUI();
  noLoop();
...
void valuesUpdated(){//updated all the values of the plane when changed in the GUI
 ...
 runwayLength = Rway_Text.getValueI();
